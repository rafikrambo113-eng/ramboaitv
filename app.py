...
if uploaded_file is not None:
    ...
    if is_modern:
        ...
        # ══════════════════════════════════════════════════════════════
        # 🔧 معالجة الشاشات القديمة <ITEM> — النسخة المُصلحة
        # المشكلة السابقة: التعديل على item_str لا يُحدّث raw_str داخل channels_to_sort
        # لأن Python strings غير قابلة للتعديل المرجعي (immutable).
        # الحل: نحفظ النص المعدّل مباشرةً في raw_str من اللحظة الأولى.
        # كذلك: استخدام re.DOTALL لضمان التقاط items متعددة الأسطر.
        # وأخيراً: بناء XML النهائي بإحلال المنطقة كاملة بدلاً من find/rfind الهش.
        # ══════════════════════════════════════════════════════════════

        # استخراج كل ITEM مع re.DOTALL لدعم الأسطر المتعددة
        ...
        for item_str in item_blocks:
            ...
            # ✅ نعمل على نسخة معدّلة من item_str ونحفظها مباشرةً
            ...
            if update_freq and name_up in NILESAT_LIVE_DB:
                ...
                if old_freq != live_freq:
                    ...
                    # ✅ التحديث يتم على updated_item_str وليس item_str
                    updated_item_str = re.sub(
                        r'<frequency>\d+</frequency>',
                        f'<frequency>{live_freq}</frequency>',
                        updated_item_str,
                        flags=re.IGNORECASE
                    )
                    old_freq = live_freq
            ...
            channels_to_sort.append({
                "name": ch_name,
                "freq": old_freq,
                "raw_str": updated_item_str,  # ✅ نحفظ النص المعدّل
                "is_injected": False
            })

        # زرع القنوات الجديدة للأنظمة القديمة
        if add_new_ch and item_blocks:
            sample_item = item_blocks[0]
            for db_name, db_info in NILESAT_LIVE_DB.items():
                if db_name not in existing_names_upper:
                    new_item = sample_item
                    new_item = re.sub(r'<vchName>.*?</vchName>', f'<vchName>{db_name}</vchName>', new_item, flags=re.IGNORECASE)
                    new_item = re.sub(r'<frequency>\d+</frequency>', f'<frequency>{db_info["frequency"]}</frequency>', new_item, flags=re.IGNORECASE)
                    channels_to_sort.append({
                        "name": db_name,
                        "freq": str(db_info["frequency"]),
                        "raw_str": new_item,
    ...
    if is_modern:
        ...
        # ══════════════════════════════════════════════════════════════
        # 🔧 بناء XML الشاشات القديمة — النسخة المُصلحة الكاملة
        #
        # المنطق الجديد:
        # 1. نبني كل ITEM من raw_str المعدّل مسبقاً (يحتوي التردد الصحيح).
        # 2. نحدّث prNum داخل كل ITEM بالترقيم الجديد.
        # 3. نستبدل منطقة الـ ITEMs كاملة في الملف الأصلي باستخدام
        #    regex دقيق يمسك من أول <ITEM> لآخر </ITEM>.
        # ══════════════════════════════════════════════════════════════

        item_strings_final = []
        for index, ch in enumerate(channels_sorted, start=1):
            raw = ch["raw_str"]  # ✅ هذا هو النص المعدّل بالفعل (تردد محدّث)

            # تحديث prNum بالرقم الجديد
            if re.search(r'<prNum>', raw, re.IGNORECASE):
                raw = re.sub(
                    r'<prNum>\d+</prNum>',
                    f'<prNum>{index}</prNum>',
                    raw,
            ...
            item_strings_final.append(raw)
            tag_status = " [NEW] " if ch["is_injected"] else ""
            text_report += f"No. {index:03d} : {ch['name']:<25} | Freq: {ch['freq']}{tag_status}\n"

        # دمج كل الـ ITEMs في نص واحد
        combined_items_str = "\r\n".join(item_strings_final)

        # ✅ استبدال منطقة ITEMs كاملة بـ regex دقيق (أول ITEM → آخر /ITEM)
        # re.DOTALL لدعم الأسطر المتعددة، re.IGNORECASE للتوافق
        final_text_output = re.sub(
            r'<ITEM>.*</ITEM>',
            combined_items_str,
            file_text,
            count=1,
