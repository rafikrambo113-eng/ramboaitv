        ...
        for row in rows:
            ...
            if len(cols) >= 3:
                ...
                if ch_name and freq > 1000:
                    live_db[ch_name] = {"frequency": freq, "polarization": "Horizontal" if polarity == "H" else "Vertical"}
...
with col_fetch_status:
    if st.session_state.live_db_cache:
        ...
        st.markdown(f"<div class='live-badge'>🟢 ✅ تم جلب بيانات NileSat الحية! إجمالي القنوات: {n:,} | ⏱ {lft}</div>", unsafe_allow_html=True)
    else:
...
def ai_classify(channel_name):
    ...
    if any(w in name for w in ["QURAN","RAHMA","MAJD","MAKKA","IQRAA","IQRA","HUDA","WESAL","ISLAM","SUNNAH","DOHAT","AL SALAM QURAN","AL SALAM SUNNAH","MENHAG","RAWHANYAT","ALKAFEEL","KUNUZ","KUNOUZ","AL NUJABA","ZITOUNA","ALGHADEER"]):
...
def build_final_xml_bytes(original_file_text, new_json_str):
    try:
        ...
        new_text = re.sub(pattern, lambda m: m.group(1) + new_json_str + m.group(3), original_file_text, flags=re.DOTALL)
...
with col_up:
    uploaded_file = st.file_uploader("🚀 اختر ملف القنوات (GlobalClone00001.TLL) من الفلاشة:", type=["TLL"], key=f"p1_uploader_{st.session_state.p1_uploader_key}")
with col_reset:
    st.write("")
    st.write("")
...
if uploaded_file is None:
    ...
    with col_chk1:
        ...
        if scan_active and not st.session_state.get('scan_done_p1', False):
            if is_modern:
                ...
                current_names_set = {ch.get("channelName", "").upper() for ch in _tmp_bd.get("channelList", [])}
            else:
            ...
            for db_name_upper, db_info in ACTIVE_DB.items():
                is_present = any(db_name_upper in existing or existing in db_name_upper for existing in current_names_set)
                if not is_present:
    ...
    with col_chk2:
        ...
        if maint_active and not st.session_state.get('maint_done_p1', False):
            ...
            if is_modern:
                ...
                for m in re.finditer(r'<vchName>(.*?)</vchName>.*?<frequency>(\d+)</frequency>', file_text, re.DOTALL):
                    ch_name = m.group(1)
                    old_f   = m.group(2)
                    name_up = ch_name.upper()
                    db_match = ACTIVE_DB.get(name_up) or ACTIVE_DB.get(next((k for k in ACTIVE_DB if k in name_up or name_up in k), ""), None)
        ...
        if maint_active:
            if st.session_state.get('maint_details_p1'):
                ...
                st.markdown("<div style='color:#888;margin-top:10px;'>ℹ️ جميع الترددات الحالية مطابقة لأحدث نسخة.</div>", unsafe_allow_html=True)
    ...
    if is_modern:
        ...
        for nc in extra_chs:
            if nc['name'].upper() not in existing_names_upper:
                ...
                new_node.update({"channelName": nc['name'], "frequency": nc['frequency'], "polarization": nc['polarization'], "Invisible": False, "skipped": False, "deleted": False, "userSelCHNo": True, "category": ai_classify(nc['name'])})
        ...
        for item_str in item_blocks:
            ...
            if name_up in freq_patches:
                new_f    = freq_patches[name_up]
                item_str = re.sub(r'<frequency>\d+</frequency>', f'<frequency>{new_f}</frequency>', item_str)
                live_freq = new_f
            else:
        ...
        if item_blocks:
            ...
            for nc in extra_chs:
                if nc['name'].upper() not in existing_names_upper:
                    new_item = re.sub(r'<vchName>.*?</vchName>', f'<vchName>{nc["name"]}</vchName>', sample_item)
    ...
    if search_query:
        results = [{"الرقم": idx, "اسم القناة": ch["name"], "الفئة": ai_classify(ch["name"]), "التردد": ch["freq"]} for idx, ch in enumerate(channels_to_sort, 1) if search_query in ch["name"].upper()]
    ...
    if is_modern:
        ...
        # ✅ LEGACY: لا نعدل prNum — هو ID القناة على القمر مش رقم ترتيب
        item_strings_sorted = []
        for ch in channels_sorted:
            item_strings_sorted.append(ch["raw_str"])
            text_report += f"No. {channels_sorted.index(ch)+1:03d} : {ch['name']:<25} | Freq: {ch['freq']}"
            text_report += " [NEW]\n" if ch["is_injected"] else "\n"
    ...
    with col_d1:
        st.download_button(label="📥 تحميل ملف الشاشة النهائي (GlobalClone00001.TLL)", data=final_xml_bytes, file_name="GlobalClone00001.TLL", mime="application/octet-stream", use_container_width=True)
    with col_d2:
        st.download_button(label="📄 تحميل تقرير الترتيب (Channels_List.txt)", data=text_report, file_name="Channels_List.txt", mime="text/plain; charset=utf-8", use_container_width=True)
    with col_d3:
