...
if uploaded_file is None:
    ...
    if is_modern:
        # ✅ الإصلاح: re.sub مباشرة بدون ET.tostring (يكسر JSON بـ XML escape)
        final_list_modern = []
        ...
        final_xml_bytes = new_text.encode('utf-8')
        if final_xml_bytes.startswith(b'\xef\xbb\xbf'):
            final_xml_bytes = final_xml_bytes[3:]

    else:
        # ✅ الإصلاح: لا نعدل prNum — هو ID القناة على القمر مش رقم ترتيب
        item_strings_sorted = []
        for index, ch in enumerate(channels_sorted, start=1):
