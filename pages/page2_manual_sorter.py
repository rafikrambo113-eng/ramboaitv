...
# ─────────────────────────────────────────────
# 1. تهيئة الجلسة (Session State) بشكل مستقل
# ─────────────────────────────────────────────
...
if 'live_db_cache' not in st.session_state:
    st.session_state.live_db_cache = None
if 'live_db_last_fetch' not in st.session_state:
    st.session_state.live_db_last_fetch = None
...
# ─────────────────────────────────────────────
# 4. قاعدة البيانات الاحتياطية + دالة الجلب الحي
# ─────────────────────────────────────────────
FALLBACK_NILESAT_DB = {
...
def fetch_nilesat_live_db():
    """يجلب قائمة قنوات NileSat الحية من dthsat.com بدون مكتبات خارجية"""
    try:
        headers = {
            "User-Agent": (
...
# ─────────────────────────────────────────────
# 5. زر الجلب الحي — يظهر دائماً في الأعلى
# ─────────────────────────────────────────────
    ...
    if st.button(t['btn_fetch_live'], use_container_width=True):
        with st.spinner(t['fetching']):
            ...
            if result:
                ...
                # إعادة تعيين الفحص والصيانة عشان يأخذوا البيانات الجديدة
                st.session_state.scan_done_p2      = False
                st.session_state.maint_done_p2     = False
                st.session_state.inserted_list_p2  = []
                st.session_state.maint_details_p2  = []
                n = len(result)
                st.toast(f"🛸 {'تم جلب' if st.session_state.lang == 'ar' else 'Fetched'} {n:,} {'قناة من NileSat!' if st.session_state.lang == 'ar' else 'channels from NileSat!'}")
                st.rerun()
            else:
...
# ─────────────────────────────────────────────
# 6. دالة قراءة وتفكيك ملف الـ TLL
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# 7. رفع الملف + زر إعادة التهيئة في الأعلى
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# 9. خيارات الفحص والصيانة التلقائية — بالقاعدة الحية
# ─────────────────────────────────────────────
...
with col_chk1:
    ...
    if scan_active and not st.session_state.get('scan_done_p2', False):
        ...
        for db_name_upper, db_info in ACTIVE_DB.items():
            ...
            if not is_present:
                ...
                freq_val = db_info["frequency"]
                ch_name  = db_name_upper.title()
                ...
                if st.session_state.is_modern:
                    ...
                    new_ch = {"id": new_idx, "name": ch_name, "freq": str(freq_val),
                              "pol": pol_full, "raw_node": node}
                else:
                    raw = (
                        f"<ITEM>\r\n<prNum>{new_idx+1}</prNum>\r\n"
                        f"<vchName>{ch_name}</vchName>\r\n"
                        f"<frequency>{freq_val}</frequency>\r\n</ITEM>"
                    )
                    new_ch = {"id": new_idx, "name": ch_name, "freq": str(freq_val),
                              "pol": pol_full, "raw_str": raw}

                st.session_state.channels.append(new_ch)
                new_inserted_names.append(
                    f"📡 {ch_name} "
                    f"({'تردد' if st.session_state.lang == 'ar' else 'Freq'}: {freq_val} {pol_full[0]})"
                )
                added_count += 1
    ...
    if scan_active:
        if st.session_state.get('inserted_list_p2'):
            ...
            label = (f"**✨ {len(injected)} قناة جديدة تم زرعها في (1. جدول القنوات الكلي):**"
                     if st.session_state.lang == 'ar'
                     else f"**✨ {len(injected)} new channels injected into the pool:**")
            st.markdown(label)
            ...
            st.markdown(
                "<div style='color:#888;margin-top:10px;'>ℹ️ "
                + ("لم يتم العثور على قنوات جديدة للزرع (مضافة بالفعل)."
                   if st.session_state.lang == 'ar'
                   else "No new channels found to inject (already present).")
                + "</div>", unsafe_allow_html=True
            )
...
with col_chk2:
    ...
    if maint_active and not st.session_state.get('maint_done_p2', False):
        ...
        for ch in st.session_state.channels:
            ...
            if db_match:
                ...
                if old_freq != new_f:
                    ...
                    detail = (
                        f"🔄 **{ch_name}** | "
                        f"{'من' if st.session_state.lang == 'ar' else 'from'} `{old_freq}` "
        ...
        if updated_count > 0:
            st.toast("🔧 " + (f"تم تحديث {updated_count} تردد!" if st.session_state.lang == 'ar'
                              else f"Updated {updated_count} frequencies!"))
            st.rerun()
    ...
    if maint_active:
        if st.session_state.get('maint_details_p2'):
            ...
            label = (f"**🔧 {len(details)} تردد تم تحديثه من القاعدة الحية في جدول المتوفر:**"
                     if st.session_state.lang == 'ar'
            ...
            st.markdown(
                "<div style='color:#888;margin-top:10px;'>ℹ️ "
                + ("جميع الترددات الحالية بجدول المتوفر مطابقة لأحدث نسخة."
                   if st.session_state.lang == 'ar'
                   else "All current frequencies match the latest version.")
                + "</div>", unsafe_allow_html=True
            )

# ─────────────────────────────────────────────
# 10. دوال الـ Callbacks
# ─────────────────────────────────────────────
...
with col_table1:
    ...
    filtered_pool = [c for c in st.session_state.channels
    ...
    display:flex;font-weight:bold;color:#00f0ff;text-align:center;'>
        ...
        <div style='flex:3;'>{'اسم القناة' if st.session_state.lang == 'ar' else 'Channel'}</div>
...
with col_table2:
    ...
    if ord_list:
        ...
        display:flex;font-weight:bold;color:#ff007f;text-align:center;margin-bottom:10px;'>
            ...
            <div style='flex:2.5;'>{'اسم القناة' if st.session_state.lang == 'ar' else 'Channel'}</div>
            ...
            for i, ch in enumerate(ord_list):
                ...
                with col_del:
                    if st.button("🗑️", key=f"del_btn_{i}_{ch['id']}",
                                 help="حذف القناة فوراً من قائمة الترتيب"):
                        st.session_state.ordered_channels.pop(i)
                        st.session_state.edit_finished = False
                        st.toast(f"🗑️ تم حذف [{ch.get('name')}] من الترتيب!")
                        st.rerun()
...
if not final_out_list:
    st.warning("⚠️ " + ("جدولك المخصص فارغ حالياً! قم بزرع قنوات أولاً لتفعيل روابط التحميل."
                        if st.session_state.lang == 'ar'
                        else "Your custom list is empty! Inject channels first to enable download links."))
else:
    ...
    if st.session_state.edit_finished:
        ...
        if st.session_state.is_modern:
            ...
            final_text = file_text[:start_i] + combined + file_text[end_i:] if start_i != -1 else combined
            try:
        ...
        st.markdown("""
        <div style="background-color:rgba(255,165,0,0.12);border-left:5px solid #ffa500;
        padding:20px;border-radius:12px;margin-top:25px;">
            <h4 style="color:#ffa500;margin-top:0;font-weight:bold;">
            💡 ملحوظة فنية هامة جداً بعد تنزيل الملف على شاشة LG:</h4>
            ...
            <ol style="font-size:15px;line-height:1.7;margin-right:20px;">
                ...
                <li>ستظهر لك القنوات المرتبة ويكون بعضها في وضع مخفي،
                قم <b>بتحديد كل القنوات</b> واختار <b>استعادة (Restore)</b>.</li>
            </ol>
